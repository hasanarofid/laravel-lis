...
(function ($) {
    ...
    //print status
    $(document).on('click', '.print_status', function () {
        var group_id = $(this).attr('group_id');
        var status_id = $(this).attr('status_id');
        $("#status_edit").val(status_id);
        // alert(status_id);
        $('#print_status_form').attr('action', url('admin/medical_reports/print_status/' + group_id));
    });

    $(document).on('submit', '#print_status_form', function () {
        $('#print_status_modal').modal('toggle');
    });
